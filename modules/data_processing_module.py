    ...
    if housekeeper_data['count'] >= 6:
        ...
        if next_reward:
            if next_reward == "达标奖":
                ...
                next_reward_gap = f"距离 {next_reward} 还需 {round(100000 - amount, 2):,} 元"
...
# 2025年2月，北京. 幸运数字8，单合同金额1万以上和以下幸运奖励不同；节节高三档；合同金额10万以上按10万计算
def determine_rewards_feb_beijing(contract_number, housekeeper_data, current_contract_amount):
    ...
    if housekeeper_data['count'] >= 6:
        ...
        if amount >= 160000 and '精英奖' not in housekeeper_data['awarded']:
            ...
            reward_names.append("精英奖")
            housekeeper_data['awarded'].append('精英奖')
        elif amount >= 100000 and '优秀奖' not in housekeeper_data['awarded']:
            # 优秀奖
            next_reward = "精英奖"
            reward_types.append("节节高")
            reward_names.append("优秀奖")
        ...
        if '达标奖' not in housekeeper_data['awarded'] and amount >= 60000:
            ...
            reward_names.append("达标奖")
            housekeeper_data['awarded'].append('达标奖')
        if '优秀奖' not in housekeeper_data['awarded'] and amount >= 100000:
            reward_types.append("节节高")
            reward_names.append("优秀奖")
            housekeeper_data['awarded'].append('优秀奖')
        ...
        if not next_reward:
            if '优秀奖' in housekeeper_data['awarded'] and  amount < 160000 and  not set(["精英奖"]).intersection(housekeeper_data['awarded']):
                next_reward = "精英奖"
            elif '达标奖' in housekeeper_data['awarded'] and  amount < 100000 and  not set(["精英奖", "优秀奖"]).intersection(housekeeper_data['awarded']):
                next_reward = "优秀奖"
...
# 2025年2月，北京. 幸运数字6，单合同金额1万以上和以下幸运奖励不同；节节高三档；合同金额10万以上按10万计算
def process_data_feb_beijing(contract_data, existing_contract_ids, housekeeper_award_lists):
    ...
    for contract in contract_data:
        ...
        performance_amount = min(current_contract_amount, config.PERFORMANCE_AMOUNT_CAP_BJ_FEB)  # 使用配置的上限值
        ...
        # 计算奖励类型和名称
        reward_types, reward_names, next_reward_gap = determine_rewards_feb_beijing(contract_count_in_activity, housekeeper_contracts[housekeeper], current_contract_amount)
...
def process_data_apr_beijing(contract_data, existing_contract_ids, housekeeper_award_lists):
    ...
    for contract in contract_data:
        ...
        # 计算奖励类型和名称
        reward_types, reward_names, next_reward_gap = determine_rewards_apr_beijing(contract_count_in_activity, housekeeper_contracts[housekeeper], current_contract_amount)
...
def process_data_shanghai(contract_data, existing_contract_ids, housekeeper_award_lists):
    ...
    for contract in contract_data:
        ...
        reward_types, reward_names, next_reward_gap = determine_rewards_shanghai_mar(contract_count_in_activity, housekeeper_contracts[unique_housekeeper_key])
...
# 3月份的奖励规则
def determine_rewards_shanghai_mar(contract_number, housekeeper_data):
    reward_types = []
    reward_names = []
    next_reward_gap = ""  # 下一级奖励所需金额差
